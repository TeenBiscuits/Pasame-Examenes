...
\`\`\``,
		explanation: `\`\${SSH_THRESHOLD:-10}\` usa valor por defecto si no está definida. Validación con regex \`^[0-9]+$\`. \`ss -t state established\` lista conexiones TCP establecidas. \`grep -c\` cuenta las que tienen puerto ssh como local.`,
	},
