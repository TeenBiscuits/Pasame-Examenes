...
import {
	getSelectedSubjectBuildStats,
	type SubjectBuildStats,
	type TopicBuildStats,
...
export default function SubjectHome({
	...
	const examStats = useMemo(
		() => new Map(Object.entries(buildStats.examStats)),
		[buildStats.examStats],
	);
	const allExamSourcesSelected =
		selectedExamIds.length ===
		(subject?.exams.filter((exam) => !exam.deleteRights).length ?? 0);
	const selectedStats = useMemo(
		() =>
			allExamSourcesSelected
				? {
						questionCount: buildStats.questionCount,
						repeatedQuestionCount: buildStats.repeatedQuestionCount,
						topicStats: buildStats.topicStats,
					}
				: getSelectedSubjectBuildStats(buildStats, selectedExamIds),
		[allExamSourcesSelected, buildStats, selectedExamIds],
	);
	const progress =
	...
	const repeatedCount = selectedStats.repeatedQuestionCount;
	...
	const description = descriptionTemplate
		.replace("{count}", String(selectedStats.questionCount))
	...
	return (
		<div className="animate-fade-in animate-duration-fast">
			...
			<div className="mx-auto max-w-6xl px-4 pb-8">
				...
				<TopicsSection
					subject={subject}
					topicStats={selectedStats.topicStats}
					progress={progress}
...
function TopicsSection({
	...
	const t = useT();
	const topicKeysWithQuestions = new Set<string>();
	for (const [topicKey, stats] of Object.entries(topicStats)) {
		if (stats.questionCount > 0) {
			topicKeysWithQuestions.add(topicKey);
		}
	}
	...
	const renderTopicCard = (topic: Topic) => {
		const stats = topicStats[topic.key] ?? { questionCount: 0, points: 0 };
